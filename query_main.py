...
def main(var, K, p, total_run, epsilon, metric_name, network_size, model_name):
    ...
    data_folder='/msmarco-data/indices/'
    ...
    for year in years:
        ...
        main_query_path = f'/msmarco-data/trec_data/{year}/{year}_queries'

        NNQ_query_path_emb =f"/NNQ/output/top_100_emb_sim_train_{year}_queries_msmarco-MiniLM-L6-cos-v5_matched_queries.json"
        with open(NNQ_query_path_emb, 'r') as json_file:
            NNQ_data_emd_similarity = json.load(json_file)
        
        
        noisy_run_path = f'/run/{metric_name}_{network_size}_{epsilon}_{var}/{metric_name}_{year}_{var}_Noisyq_run.tsv'
        
        # write a code to check if a folder exits or not, if not create it.
        folder_path = f'/run/{metric_name}_{network_size}_{epsilon}_{var}'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        
        if not os.path.exists(f'/RBO/{metric_name}_{network_size}_{epsilon}_{var}'):
            os.makedirs(f'/RBO/{metric_name}_{network_size}_{epsilon}_{var}')
        ...
        for n in range(total_run):
            ...
            orig_run_path = f'/run/org_{year}.tsv'

            RBO_func(orig_run_path, noisy_run_path, RBO_output_path, year, p, K )
            
            performance_path = f'/performance/ndcg_cut_10_dl{year}.tsv'
    ...
    if not os.path.exists(f"/correlations/{metric_name}"):
        os.makedirs(f"/correlations/{metric_name}")

    with open(f"/correlations/{metric_name}/{metric_name}_{network_size}_{epsilon}_{var}.txt", 'w') as out:
